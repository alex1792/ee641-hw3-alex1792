...
def evaluate_extrapolation(model, data_dir, test_lengths, device, batch_size=32):
    ...
    extrapolation_lengths = [32, 64, 128, 256]

    for length in test_lengths:
        print(f"Testing on length {length}...")

        # Check if this is an extrapolation length
        if length in extrapolation_lengths:
            # Use extrapolation dataset
            dataloader = create_extrapolation_loader(
                data_dir, length, batch_size
            )
        else:
            # For training lengths, use regular test dataset and filter by length
            from dataset import create_dataloaders
            _, _, test_loader = create_dataloaders(data_dir, batch_size)
            
            # Filter test data to only include sequences of target length
            filtered_sequences = []
            filtered_labels = []
            filtered_lengths = []
            filtered_masks = []
            ...
            for batch in test_loader:
                for i in range(batch['sequence'].size(0)):
                    if batch['length'][i].item() == length:
                        filtered_sequences.append(batch['sequence'][i])
                        filtered_labels.append(batch['label'][i])
                        filtered_lengths.append(batch['length'][i])
                        filtered_masks.append(batch['mask'][i])
            
            if len(filtered_sequences) == 0:
                print(f"  No data found for length {length}, skipping...")
                continue
            
            # Create a dataloader from filtered data
            from torch.utils.data import DataLoader, TensorDataset
            sequences_tensor = torch.stack(filtered_sequences)
            labels_tensor = torch.stack(filtered_labels)
            lengths_tensor = torch.stack(filtered_lengths)
            masks_tensor = torch.stack(filtered_masks)
            
            dataset = TensorDataset(sequences_tensor, labels_tensor, lengths_tensor, masks_tensor)
        ...
        with torch.no_grad():
            for batch in tqdm(dataloader, desc=f"Length {length}"):
                # Handle both batch formats - check type, not length!
                if isinstance(batch, (list, tuple)) and len(batch) == 4:
                    # TensorDataset format: (sequences, labels, lengths, masks)
                    sequences, labels, lengths, masks = batch
        ...
        if total > 0:
            ...
            print(f"  Accuracy: {accuracy:.2%} (samples: {total})")
        else:
            print(f"  No samples found for length {length}")
...
def visualize_learned_positions(model_path, output_dir, max_positions=128):
    ...
    embeddings = pos_encoding.position_embeddings.weight.data.numpy()

    # Visualize embeddings as heatmap
    plt.figure(figsize=(12, 8))

    # Create heatmap of position embeddings
    # Show first max_positions positions and all dimensions
    im = plt.imshow(embeddings[:max_positions, :], cmap='RdBu_r', aspect='auto')
